...
`"Isotòpia Animals (19.09.2023)"` resource:


```{r getdata}
isotopicData <- getData(
  name = "Isotòpia Animals (19.09.2023)") 
isotopicData  %>% 
  head(5)  %>% 
  dplyr::select(c("ID", "General_Category/Family", "Environment", "Trophic_Category", "Substratus")) %>%
  knitr::kable()
```
For text files one can specify e.g. the separator using the helper function `dataOptions()`:

```{r getdataWithOptions, eval = os_linux}
isotopicData <- getData(
  name = "CIMA Humans 29.05.2021 CSV",
  options = dataOptions(sep = ";")) 
isotopicData  %>% 
  head(5)  %>% 
  dplyr::select(c("Entry_ID", "Individual_ID", "Min_Age_yrs", "Max_Age_yrs", "Sampled_Element", "Analysed_Component")) %>%
  knitr::kable()
```
